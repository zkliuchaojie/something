...
int set_add(HashTable *set, Value_t val, void *not_used_for_now) {
    set->Insert(val, val);
    return 1;
}

int set_remove(HashTable *set, Value_t val, void *not_used_for_now) {
    set->Delete(val);
    return 1;
}

int set_contains(HashTable *set, Value_t val, void *not_used_for_now) {
...
unsigned long long set_size(HashTable *intset) {
    return intset->Size();
}

void set_delete(HashTable *intset) {
    delete intset;
}
