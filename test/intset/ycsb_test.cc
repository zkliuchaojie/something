...
int set_update(AbstractIntset *set, Value_t val, void *not_used_for_now) {
...
#define NR_LOAD         (1000000UL)        // the number of operations in load_ycsb
#define NR_OPERATIONS   (10000000UL)    // the data you generated must be greater or equal to this
#define LOAD_YCSB      "ycsb_test/d_load_ok"      //  ycsb load file
#define RUN_YCSB       "ycsb_test/d_run_ok"       // ycsb workload file
...
static void *test(void *data)
{
  ...
  Key_t *runkeys = d->runkeys;
  Key_t *operationtypes = d->operationtypes;
  /* Wait on barrier */
  barrier_cross(d->barrier);
  int i = 0;
  while (stop == 0) {
    if(operationtypes[i] == 1) {
      set_add(d->set, runkeys[i], d);
      d->nb_add++;
    } else if (operationtypes[i] == 2) {
      if (set_contains(d->set, runkeys[i], d))
        d->nb_found++;
      d->nb_contains++;
    } else if (operationtypes[i] == 3) {
	set_update(d->set, runkeys[i], d);
        d->nb_update++;
    }
    i++;
    if (i >= NR_OPERATIONS)
...
int main(int argc, char **argv)
{
  ...
  init_vmp();
    ...
    for(int i=0; i<NR_OPERATIONS; ++i) {
        ...
        {
             operationtypes[i] = 2;
        } else if (operation.compare(update_op) == 0)
        {
                operationtypes[i] = 3;
        }
        else
  ...
  for (i = 0; i < nb_threads; i++) {
    ...
    data[i].runkeys = runkeys;
    data[i].operationtypes = operationtypes;
  ...
  for (i = 0; i < nb_threads; i++) {
    ...
    updates += (data[i].nb_add + data[i].nb_update);
    size += data[i].diff;
  }
