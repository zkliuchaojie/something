...
public: 
    ...
    virtual void Undo(AbstractPtmObject *object) = 0;
...
public:
    ...
    void Push(AbstractPtmObjectWrapper *ptm_object_wrapper, unsigned long long ti_and_ts, AbstractPtmObject *object) {
        ...
        clflush((char *)(set_[entries_num_].object_), 64);
        mfence();
    ...
    void Undo() {
	    //std::cout << entries_num_ << std::endl;
	    for (int i=0; i<entries_num_; i++) {
            set_[i].ptm_object_wrapper_->Undo(set_[i].object_);
        }
    }
    void Unlock() {
        for (int i=0; i<entries_num_; i++) {
...
public:
    ...
    void Undo(AbstractPtmObject *object) {
        object_.Copy(object);
        clflush((char *)this, sizeof(*this));
    }
    void Unlock() {
